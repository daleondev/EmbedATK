...
public:
    ...
    protected: 
        std::span<uint8_t> m_stack;
        std::function<void()> m_task;
    private:
        void setStack(std::span<uint8_t> stack) { m_stack = stack; }
        void setTask(std::function<void()>&& task) { m_task = std::move(task); }
        friend class OSAL;
    };
    template<typename Callable, typename... Args>
    static void createThread(IPolymorphic<Thread>& thread, std::span<uint8_t> stack, Callable&& func, Args&&... args) 
    { 
    ...
    template<typename Callable, typename... Args>
    static void createCyclicThread(IPolymorphic<CyclicThread>& cyclicThread, std::span<uint8_t> stack, Callable&& func, Args&&... args) 
    { 
