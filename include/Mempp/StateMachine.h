...
public:
    StateMachine()
    {
        ...
        m_prevStatePath = m_activeStatePath;
    ...
    constexpr auto previousState() const { return m_prevStatePath[m_prevStatePath.size() - 1]; }
...
private:
    ...
    void executeTransition() 
    {
        if constexpr (IS_HIERARCHICAL) {
            ...
            // Action
            if constexpr (!std::is_same_v<std::decay_t<decltype(T::action)>, std::nullopt_t>) {
                if constexpr (is_optional_v<decltype(T::action)>) {
                    if (T::action) (*T::action)(T::from, T::trig, T::to);
                }
                else {
                    T::action(T::from, T::trig, T::to);
                }
            }
