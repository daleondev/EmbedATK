...
public:
	...
	virtual ~LinuxNetworkAdapter() = default;

	static std::expected<void, std::string> getNetworkAdapters(INetworkAdapter::Adapters& adapters)
	{
...
static_assert(sizeof(LinuxNetworkAdapter) <= INetworkAdapter::NETWORK_ADAPTER_ALLOC_DATA.size, "not enough data allocated -> increase size");
static_assert(alignof(LinuxNetworkAdapter) == INetworkAdapter::NETWORK_ADAPTER_ALLOC_DATA.align, "invalid alignment for allocation");
